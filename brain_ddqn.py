...
class Brain:
    ...
    def replay_new(self):
        ...
        action_batch = torch.cat(transition.action).to(device)
    ...
    def decide_actions(self, states, episodes = None):
        ...
        if self.eps_scheduler is not None:
            ...
            eps_batch = np.zeros((states.shape[0],1))
        if eps_batch == 0: #pick from model
            self.model.eval()
            with torch.no_grad():
                actions = self.model(states.to(self.device)).argmax(1).view(states.shape[0], 1)
            return actions
        actions = np.where(eps_batch <= np.random.uniform(0,1), self.model(states.to(self.device)).argmax(1).view(states.shape[0], 1).cpu(),
                           torch.LongTensor([np.random.random_integers(0, self.num_actions, size = (states.shape[0],))]).view(states.shape[0],1))
...
class EpsilonAnnealing:
    ...
    def get_batch(self, episode = None):
        ...
        return np.where(episode <= self.observe, 0, (1 / (episode - self.observe + 1)).clip(self.min_val))
